...
namespace PepperDash.Essentials.Plugin
{
	...
	/// destination, rather than assumed to be a fixed direct physical connection.
	/// </summary>
	public class Nhd150Rx : NhdBaseDevice, IRoutingSource
	{
		...
		public bool ApplyDynamicLayout(
			IReadOnlyList<NhdDynamicMultiviewLayoutCalculator.ParticipantSource> participantSources,
		...
		/// <see cref="ApplyDynamicLayout(IReadOnlyList{NhdDynamicMultiviewLayoutCalculator.ParticipantSource}, string)"/>.
		...
		/// <see cref="IReadOnlyList{T}"/> of the <c>ParticipantSource</c> POCO. This takes parallel
		/// primitive arrays instead (<paramref name="sourceKeys"/>[i] paired with
		...
		public bool ApplyDynamicLayout(string[] sourceKeys, int[] priorities, string presentationSourceKey)
		{
			...
			var participantSources = sourceKeys
				.Select((key, i) => new NhdDynamicMultiviewLayoutCalculator.ParticipantSource(key, priorities[i]))
				.ToList();
